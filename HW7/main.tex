...
We can actually extend this result to any interval centered at $2\pi m$ of width $2 \pi$ by noticing that 
\begin{eqnarray}
    \int_{a-\pi}^{a+\pi} e^{i(n-n')x} = 2\pi \delta_{nn'}
.\end{eqnarray}
Thus, on an interval $[(2m-1)\pi,(2m+1)\pi]$
\begin{eqnarray}
...
Since this result is independent of $m$, we can generally apply this result for any $x \in \reals$, meaning that the Dirac comb is a sum of complex exponentials.
...
.\end{eqnarray}
This could actually be summed up exactly, which is nice since this was not clear from the original form of the series, but this will be left for a later exercise since the focus of this problem was on the Fourier decomposition of the function.
...
\begin{eqnarray}
    \begin{aligned}
        I_6(x) &= \sum_{n=1}^{\infty} e^{-kn} \sin{nx} = \sum_{n=0}^{\infty} e^{-kn}\sin{nx} \\
               &= \Im \Big\{ \sum_{n=0}^{\infty} e^{(-k+ix)n} \Big\} = \eqbox{ \frac{e^{-k} \sin{x}}{e^{-2k} \sin^2{x} + (1 - e^{-k} \cos{x})^2} }
    .\end{aligned}
\end{eqnarray}
...
.\end{align}
Notice that we have put in the absolute value by hand since the result should be independent of the signs of $a$ and $b$, and we must satisfy $C(\omega) = C^{*}(-\omega)$\footnote{The point about the absolute value of $a$ and $b$ is because $z^2 + a^2 = (z + i|a|)(z - i|a|)$, and the pole in the upper half-plane is $z = i|a|$.}
...
\begin{aligned}
    I_1(x) &= \frac{1}{2 \pi} \int_{-\infty}^{\infty} C(\omega) e^{-i \omega x} \dd{\omega} = \frac{1}{2} \int_{0}^{\infty} \big[ C(\omega) e^{-i \omega x} + C(-\omega) e^{i \omega x} \big] \\
           ...
           &= \eqbox{ \int_{0}^{\infty} \Bigg[ \frac{A}{|a|} e^{-\omega|a|} \cos{\omega x} - \frac{B}{2|b|} \omega e^{-\omega|b|} \sin{\omega x} \Bigg] \dd{\omega} }
.\end{aligned}
\end{eqnarray}
...
\eqbox{ 
\begin{aligned}
    \omega^2 &= \frac{(2\omega_0^2 + \omega_{L}^2) \pm \sqrt{(2 \omega_0^2 + \omega_{L}^2)^2 - 4\omega_0^{4}}}{2} \\
             &= \omega_0^2 \Bigg[ 1 + \frac{1}{2} ( \omega_{L} /\omega_0 )^2 \pm \sqrt{1 + (\omega_{L}/\omega_0)^2} \Bigg]
\end{aligned}
}
.\end{eqnarray}
