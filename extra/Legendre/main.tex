...
Before we do so, let us take $\mathcal{N}_{l} \rightarrow (-1)^{l} \mathcal{N}_{l}$ such that
\begin{eqnarray}
    P_{l}(x) = \mathcal{N}_{l} \dv[l]{x} (x^2 - 1)^{l}
.\end{eqnarray}
This is convenient so that we can avoid doing the accounting for the extra factors of $-1$ each time we take a derivative, and we have the freedom to do this since any compensating factors will naturally arise in our later work.

Observe that by the binomial theorem ({\color{red} is it nicer to write $x^{2k}$ or $x^{2(l-k)}$})
\begin{eqnarray}
    (x^2 - 1)^{l} = \sum_{k=0}^{l} \binom{l}{k} x^{2k}
.\end{eqnarray}
Taking the $l^{\rm th}$ derivative and evaluating at $x = 1$, we have
\begin{align}
    \dv[l]{x} (x^2 - 1)^{l}\Big|_{x=1} &= \sum_{k= \lfloor l/2 \rfloor}^{l} \frac{l!}{k!(l-k)!} (2k)(2k-1)\ldots(2k-l+1) \nonumber \\
                                       &= \sum_{k=\lfloor l/2 \rfloor}^{l} \frac{l! (2k)!}{k!(l-k)!(2k-l)!} = \sum_{k=\lfloor l/2 \rfloor}^{l} \frac{l!}{(l-k)!(2k-l)!} 2^{k} (2k-1)!! \nonumber \\
.\end{align}
