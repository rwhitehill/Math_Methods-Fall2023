...
\begin{eqnarray}
    \sin{x} = x \prod_{n=1}^{\infty} \Big( 1 - \frac{x^2}{n \pi^2} \Big)
.\end{eqnarray}
From this formala, it is also trivial to glean that $\sin{x}/x$ has zeros when $\sin{x} = 0$ or $x = n \pi$ for $n = \pm 1, \pm 2, \ldots$\footnote{Note that there is not a zero when $n=0$ (i.e. $x = 0$) since the $1/x$ factor makes the function indeterminate there and $\sin{x}/x \rightarrow 1$ as $x \rightarrow 0$. Alternatively, from the product expansion, the factor $x$ out front cancels with $1/x$, and we take the limit of this as $x \rightarrow 0$, which is just an infinite product of ones.}.
...
We find that
\begin{align}
    e^{\pm i \pi/2} &= \cos{ (\pm \pi/2) } + i \sin{( \pm \pi/2 )} = \pm i \\
    e^{i \pi n} &= \cos{(\pi n)} + i \sin{(\pi n)} = (-1)^{n} \\
    \ln(-1) &= \ln(e^{i\pi}) = i \pi
.\end{align}
